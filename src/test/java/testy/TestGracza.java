...
public class TestGracza {
    ...
    void testKonstruktoraMinimalista() throws NieprawidłoweDane {
        ...
        assertEquals(środki, gracz.getŚrodkiFinansowe(), "Środki finansowe powinny być " +
                "poprawnie ustawione");
    }
    ...
    void testKonstruktoraStałoliczbowy() throws NieprawidłoweDane {
        ...
        Stałoliczbowy gracz = new Stałoliczbowy("Anna", "Nowak", 98765432, środki,
                ulubioneLiczby, kolektury);
        ...
        assertArrayEquals(ulubioneLiczby, gracz.getUlubioneLiczby(), "Ulubione liczby powinny " +
                "być poprawnie ustawione");
    }

    @Test
    void testKonstruktoraLosowy() throws NieprawidłoweDane {
        ArrayList<Kolektura> kolektury = new ArrayList<>(Arrays.asList(new Kolektura(),
                new Kolektura(), new Kolektura()));
    ...
    void testKonstruktoraStałoblankietowy() throws NieprawidłoweDane {
        ...
        assertEquals(5, gracz.getCoIleLosowań(), "Częstotliwość losowań powinna być " +
                "ustawiona na 5");
    }
    ...
    void testWypłacalności() throws NieprawidłoweDane {
        ...
        Minimalista minimalista = new Minimalista("Adam", "Małysz", 1234567890,
                new Kwota(50, 0), kolektura);
        assertTrue(minimalista.jestWypłacalny(new Kwota(30, 0)), "Minimalista " +
                "powinien być wypłacalny");
        assertFalse(minimalista.jestWypłacalny(new Kwota(100, 0)), "Minimalista " +
                "nie powinien być wypłacalny");
        ...
        assertTrue(stałoliczbowy.jestWypłacalny(new Kwota(300, 0)), "Stałoliczbowy " +
                "powinien być wypłacalny");
        assertTrue(stałoliczbowy.jestWypłacalny(new Kwota(500, 0)), "Stałoliczbowy " +
                "powinien być wypłacalny dla równej kwoty");
    }
    ...
    void testZapłać() throws NieprawidłoweDane {
        ...
        Minimalista minimalista = new Minimalista("Tomasz", "Lewandowski", 9988776,
                new Kwota(200, 0), kolektura);
        Kwota płatnośćMinimalista = new Kwota(75, 0);
        minimalista.zapłać(płatnośćMinimalista);
        assertEquals(new Kwota(125, 0), minimalista.getŚrodkiFinansowe(), "Środki " +
                "Minimalisty powinny być pomniejszone");
    }
    ...
    void testOdbierzŚrodkiNagrody() throws NieprawidłoweDane {
        ...
        assertEquals(new Kwota(5100, 0), gracz.getŚrodkiFinansowe(), "Środki " +
                "powinny zostać zwiększone o nagrodę");
    }

    @Test
    void testKupKuponMinimalista() throws NieprawidłoweDane, BrakŚrodków {
        Kolektura kolektura = new Kolektura();
        Minimalista minimalista = new Minimalista("Adam", "Mazur", 121212121,
                new Kwota(100, 0), kolektura);

        int początkowaLiczbaKuponów = minimalista.ileKuponów();
        minimalista.kupKupon();

        assertEquals(początkowaLiczbaKuponów + 1, minimalista.ileKuponów(), "Minimalista " +
                "powinien kupić jeden kupon");
    ...
    void testKupKuponStałoliczbowy() throws NieprawidłoweDane, BrakŚrodków {
        ...
        assertEquals(1, kupon.getZakłady().size(), "Kupon powinien mieć jeden zakład z " +
                "ulubionymi liczbami");
    ...
    void testKupKuponLosowy() throws NieprawidłoweDane {
        ...
        assertTrue(losowy.ileKuponów() > początkowaLiczbaKuponów, "Losowy powinien kupić " +
                "przynajmniej jeden kupon");
        assertTrue(losowy.ileKuponów() <= początkowaLiczbaKuponów + 100, "Losowy nie powinien kupić " +
                "więcej niż 100 kuponów");
    ...
    void testOddajKupon() throws NieprawidłoweDane, BrakŚrodków {
        Kolektura kolektura = new Kolektura();
        Minimalista gracz = new Minimalista("Agnieszka", "Dąbrowska", 77889900,
                new Kwota(300, 0), kolektura);
    ...
    void testKupKuponStałoblankietowy() throws NieprawidłoweDane, BrakŚrodków {
        ...
        assertEquals(1, stałoblankietowy.ileKuponów(), "Stałoblankietowy powinien kupić " +
                "jeden kupon");

        Kupon kupon = stałoblankietowy.getKupony(0);
        assertEquals(4, kupon.getNaIleLosowań(), "Kupon powinien być na tyle losowań ile" +
                " w blankiecie");
        assertEquals(2, kupon.getZakłady().size(), "Kupon powinien mieć taką samą liczbę" +
                " zakładów jak blankiet");
    }
    ...
    void testOgraniczeniaStałoliczbowy() throws NieprawidłoweDane, BrakŚrodków {
        ...
        Stałoliczbowy gracz = new Stałoliczbowy("Andrzej", "Duda", 778899001,
                new Kwota(50, 0), liczby, kolektury);

        gracz.kupKupon();
        assertEquals(1, gracz.ileKuponów(), "Pierwszy kupon powinien być kupiony");

        assertDoesNotThrow(gracz::kupKupon, "Nie powinien kupować drugiego kuponu przed " +
                "zakończeniem pierwszego");
